<?php include "css/topforuser.php" ;
session_start();
$id = $_SESSION['id'];
?>	
	<section id="about-us"  style="background-image:url(images/list.jpg);">
        <div class="container-fluid"  >			
			<div class="jumbotron"  style="background-color : transparent;color:white;"  >
    			<h1 class="display-3 text-center">COMPLAINT LIST</h1>
    		</div>

    
      <div class ="row">
      	<div class="col-md-6 col-sm-6">
    				<div class="well">
	<div class="container">
       <a   class='btn btn-default' href="createcomplaint.php"><b>Register complaint</b></a> 
                 <a  class='btn btn-default' href="userprofile.php">
                 <b>Profile </b></a><br>
                 </div>
                 </div>
                 </div>
                 </div>
                 
                 
<?php
include "crdb.php";
$query="select * from complaint_registration where User_id=$id";
$result=$link->query($query);
echo '<center><table border="1" class="table table-striped">
<tr>
<th>Complaint id</th>
<th>Complaint no</th>
<th>Suspect details</th>
<th>Description</th>
<th>Crime date</th>
<th>Crime type</th>
<th>Crime place</th>
<th colspan="3">Action</th>
</tr>';
while ($row=mysqli_fetch_row($result))
 {
	echo "<tr>";
	 echo "<td>".$row[0]."</td>";
	  echo "<td>".$row[1]."</td>";
	   echo "<td>".$row[3]."</td>";
	    echo "<td>".$row[4]."</td>";
	     echo "<td>".$row[5]."</td>";
	       echo "<td>".$row[6]."</td>";
	       echo "<td>".$row[8]."</td>";
            echo "<td><a class='btn btn-success' href='cledituser.php?id=$row[0]'>Edit</a></td>";
             echo  "<td><a class='btn btn-danger'  href='cldeleteuser.php?id=$row[0]'>Delete</a>
        </td>";
	    echo "</tr>";
	       }
	  echo "</table>";
	  
?>
</div>
</div>

</div><!--/.container-->
    </section><!--/about-us-->
  
  
<?php include "css/footer.php" ?>

